...
resource "aws_apigatewayv2_route" "cognito-api-gateway-user-delete" {
  api_id             = aws_apigatewayv2_api.cognito-api-gateway.id
  route_key          = "ANY /user/delete"
  authorization_type = "JWT"
  authorizer_id      = aws_apigatewayv2_authorizer.cognito-api-gateway.id

  target = "integrations/${aws_apigatewayv2_integration.cognito-api-gateway-user-delete.id}"
}
...
resource "aws_apigatewayv2_integration" "cognito-api-gateway-user-delete" {
  api_id           = aws_apigatewayv2_api.cognito-api-gateway.id
  integration_type = "AWS_PROXY"

  integration_uri        = aws_lambda_function.cognito-user-delete.invoke_arn
  payload_format_version = "2.0"
}
...
resource "aws_lambda_permission" "lambda-permission-cognito-user-delete" {
  statement_id  = "cognitoUserDeleteAPIInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.cognito-user-delete.arn
  principal     = "apigateway.amazonaws.com"

  # The /* part allows invocation from any stage, method and resource path
  # within API Gateway.
  source_arn = "${aws_apigatewayv2_api.cognito-api-gateway.execution_arn}/*/*/user/delete"
}
